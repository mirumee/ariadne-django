...
def execute_query(request_factory, schema, query, **kwargs):
    ...
    request = request_factory.post("/graphql/", data=query, content_type="application/json")
...
def test_custom_root_value_function_is_set_and_called_by_app(request_factory, schema):
    ...
    execute_query(request_factory, schema, {"query": "{ status }"}, root_value=get_root_value)
    get_root_value.assert_called_once()


def test_custom_root_value_function_is_called_with_context_value(request_factory, schema):
    get_root_value = Mock(return_value=True)
    execute_query(
        request_factory,
        schema,
...
def test_custom_validation_rules_function_is_called_with_context_value(request_factory, schema, validation_rule):
    get_validation_rules = Mock(return_value=[validation_rule])
    execute_query(
        request_factory,
        schema,
...
def test_extensions_function_result_is_passed_to_query_executor(request_factory, schema):
...
def test_default_logger_is_used_to_log_error_if_custom_is_not_set(request_factory, schema, mocker):
